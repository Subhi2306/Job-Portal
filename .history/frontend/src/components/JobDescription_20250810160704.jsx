...
const JobDescription = () => {
    // some ek inbuilt funcn h jiska kaam h agar koi cheez exist krti h toh woh true de deta h(some method array me use hota h )
    // applicant ek id h 

    // params ka use krke job id get krunga
   ...
   useEffect(() => {
           ...
           fetchSingleJob();
           // yah par array dependency bhi provide krni pdegi jisme jobid daalni pdegi kyuki baar baar change hogi aur userid bhi kyuki woh bhi change hogi baar baar
  ...
  return (
    <div className='max-w-7xl mx-auto my-10'>
        ...
        <div className='flex items-center gap-2 mt-4'>
            <Badge className="text-[#b10ac7] font-bold" variant="ghost">{singleJob?.position} Positions</Badge>
            <Badge className="text-[#de790e] font-bold" variant="ghost">{singleJob?.jobType}</Badge>
            <Badge className="text-[#48a808] font-bold" variant="ghost">{singleJob?.salary} LPA</Badge>
        </div>
        </div>
        <Button 
            onClick={isApplied ? null : applyJobHandler}
            disabled={isApplied} 
            className={`rounded-lg ${isApplied ? ' text-white bg-gray-600 cursor-not-allowed' : 'bg-[#7209b8] hover:bg-[#5f32ad]'}`}>
